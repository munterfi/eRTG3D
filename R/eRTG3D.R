...
qProb.3d <- function(sim, n.locs, multicore = FALSE)
{
  if (multicore) {
    ...
    return(.qProb.3d.unix(sim, n.locs, multicore = FALSE))
  }
}
...
n.sim.cond.3d <- function(n.sim, n.locs, start = c(0,0,0), end=start, a0, g0, densities, qProbs, error = FALSE, multicore = FALSE, DEM = NULL, BG = NULL)
{
  start.time <- Sys.time()
  n.sim <- round(n.sim)
  if (n.sim <= 1) {return(sim.cond.3d(n.locs, start, end, a0, g0, densities, qProbs, error, DEM, BG))}
  ...
  if (multicore) {
    ...
    if(.Platform$OS.type == "windows") {stop("Parallel version not yet supported on Windows. Please set 'multicore' to 'FALSE' or change to a unix system.")}
  } else {
    cerwList <- suppressMessages(lapply(X = 1:n.sim, FUN = function(x){sim.cond.3d(n.locs, start, end, a0, g0, densities, qProbs, error, DEM, BG)}))
  }
  message(paste("  |Runtime: ", round(as.numeric(Sys.time()) - as.numeric(start.time), 2), " secs", sep = ""))
  return(cerwList)
}
...
test.verification.3d <- function(track1, track2, alpha = 0.05, plotDensities = TRUE)
{
