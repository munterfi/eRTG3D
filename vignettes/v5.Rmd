...
The computationally most intense functions `qProb.3d()`, `n.sim.cond.3d()` and `reproduce.track.3d()` of the package are also implemented in a parallel version. On Unix systems this is done with the `parallel` package. On Windows systems, a combination of the packages `doParallel`, `parallel` and `plyr` ones is used.
...
The parallel version of the `qProb.3d()` function can be accessed by setting the parameter `multicore=TRUE`:
